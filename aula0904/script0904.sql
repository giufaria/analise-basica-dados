...
select max(salario) as media_renda, min(salario) from empregado where sexo='F'; 
select max(salario) as media_renda, min(salario) from empregado where sexo='M';
...
select replace(nome,"A","") as nome_modificado,
